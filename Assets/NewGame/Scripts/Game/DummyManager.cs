...
//		XmlNodeList reference = xDoc.GetElementsByTagName ("reference");
		...
		referenceArt.SetX(float.ToInt16(reference[0].Attributes["x"].InnerText));
		referenceArt.SetY(float.ToInt16(reference[0].Attributes["y"].InnerText));
		referenceArt.SetZ(float.ToInt16(reference[0].Attributes["z"].InnerText));
	*/		
